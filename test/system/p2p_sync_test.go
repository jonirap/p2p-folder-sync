...
func TestPeerToPeerFileSyncNetwork(t *testing.T) {
	...
	// Wrap messengers to intercept operations
	peer1.Messenger = NewNetworkOperationMonitoringMessenger(peer1.Messenger, monitor)
	peer2.Messenger = NewNetworkOperationMonitoringMessenger(peer2.Messenger, monitor)
	...
	// Connect peers explicitly for this test
	if err := nth.ConnectPeers([]*NetworkPeerSetup{peer1, peer2}); err != nil {
		t.Fatalf("Failed to connect peers: %v", err)
	}
	t.Log("SUCCESS: Peers connected over real network")

	// Give filesystem watchers time to initialize
	time.Sleep(2 * time.Second)
	...
	// Wait a bit for file sync to happen
	t.Log("Waiting for file to sync to peer2...")
	...
	// Basic network sync test completed successfully
	t.Log("SUCCESS: Network file sync test passed!")
}
